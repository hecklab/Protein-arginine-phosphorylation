...
```{r}
phospho_Stp1 <- read.delim("./pArg_proteins/Phospho (RHSTY)Sites_Stp1.txt", header = TRUE, stringsAsFactors = FALSE, dec = ".")

phospho_Stk1 <- read.delim("./pArg_proteins/Phospho (RHSTY)Sites_Stk1.txt", header = TRUE, stringsAsFactors = FALSE, dec = ".")
phospho_SdrE <- read.delim("./pArg_proteins/Phospho (RHSTY)Sites_SdrE.txt", header = TRUE, stringsAsFactors = FALSE, dec = ".")
...
pRHSTY.not.filtered<- phospho_RHSTY[, c("Protein","Amino.acid", "Positions.within.proteins", "Number.of.Phospho..RHSTY.",
                         "Intensity.SdrE_1", "Intensity.SdrE_2", "Intensity.SdrE_3", "Intensity.SdrE_4",
                         "Intensity.Stk1_1", "Intensity.Stk1_2", "Intensity.Stk1_3", "Intensity.Stk1_4",
                         "Intensity.Stp1_1", "Intensity.Stp1_2", "Intensity.Stp1_3", "Intensity.Stp1_4", "Delta.score",
...
minimal.df<- uniprotname[, c("Protein", "Protein.names", "Gene.names", "Function..CC.")]
...
qqnorm(cutoffdatam$value)
qqline(cutoffdatam$value, col="red", lty=2)
...
pRHSTY.filtered.Stp1.Ctrl.regulation<- data.frame(
  ...
  count=c(nrow(pRHSTY.filtered[pRHSTY.filtered$Stp1.SdrE.subset == "neutral",]), 
          nrow(pRHSTY.filtered[pRHSTY.filtered$Stp1.SdrE.subset == "down",]),
          nrow(pRHSTY.filtered[pRHSTY.filtered$Stp1.SdrE.subset == "up",]))
)

pRHSTY.filtered.Stk1.Ctrl.regulation<- data.frame(
  category=c("neutral", "down", "up"),
  count=c(nrow(pRHSTY.filtered[pRHSTY.filtered$Stk1.SdrE.subset == "neutral",]), 
          nrow(pRHSTY.filtered[pRHSTY.filtered$Stk1.SdrE.subset == "down",]),
          nrow(pRHSTY.filtered[pRHSTY.filtered$Stk1.SdrE.subset == "up",]))
)
...
psite.change.Stp1.Ctrl <-
  ...
  scale_fill_manual(values = c("#000000","#c51b7d", "#5AB4AD")) +
  coord_polar(theta="y", start = 0.375)+
  theme_void()+
  theme(legend.position = "none", plot.title =  element_text(hjust = 0.5))+
  ggtitle("Phosphorylation changes \n 2800")
...
psite.change.Stk1.Ctrl <-
  ...
  geom_text(x=1, aes(y=labelPosition, label=label), size=3, ) + # x here controls label position (inner / outer)
  scale_fill_manual(values = c("#000000","#c51b7d", "#5AB4AD")) +
  coord_polar(theta="y", start = 0.155)+
  theme_void()+
  theme(legend.position = "none", plot.title =  element_text(hjust = 0.5))+
  ggtitle("Phosphorylation changes \n 2800")
```
